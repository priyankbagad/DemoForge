...
/* ---- harden process so it never dies silently ---- */
...
// NOTE: add your Netlify URL below once deployed
...
app.get("/", (_req, res) => res.type("text").send("DemoForge API ✓"));
...
const ExplainResponse = z.object({
  narrative: z.string(),
  key_values: z
    .array(z.object({ label: z.string(), value: z.union([z.string(), z.number(), z.boolean()]).nullable() }))
    .default([]),
  timeline: z
    .array(z.object({ step: z.string(), detail: z.string().optional() }))
    .default([]),
...
function mockExplain({ endpoint, requestBody, responseBody, status }) {
  ...
  return {
    narrative: succeeded
      ? `Demo mode: created a sample ${amount ? `$${(Number(amount) / 100).toFixed(2)} ` : ""}object via ${
    ...
    key_values: [
      {
        label: "endpoint",
        value: `${endpoint.method || "POST"} ${(endpoint.baseUrl || "https://api.example.com") + (endpoint.path || "/endpoint")}`,
      },
      { label: "id", value: id },
...
app.post("/api/explain", async (req, res) => {
  console.log("[/api/explain] keys:", Object.keys(req.body || {}));

  // Public-friendly: demo mode short-circuits the LLM call
  if (DEMO_MODE) {
    const parsedDemo = ExplainRequest.safeParse(req.body);
    const safe = parsedDemo.success ? parsedDemo.data : { endpoint: {}, requestBody: {}, responseBody: {}, status: 200 };
    return res.json(mockExplain(safe));
  }

  // Optional gate when not in demo
  ...
  if (!parsed.success) {
    console.error("[/api/explain] Zod error:", parsed.error.issues);
...
Do not include any additional keys.`;
  ...
  const userContent = {
    ...
    content: [
      {
        type: "text",
        text: `Explain this API interaction.
  ...
  try {
    ...
    if (!ok) {
      ...
      if (notFound) {
        ...
        ({ ok, bodyText } = await callAnthropic({
    ...
    if (!safe.success) {
      console.warn("[/api/explain] LLM JSON schema mismatch; return fallback.");
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`[demoforge-llm-proxy] listening on http://0.0.0.0:${PORT}  (demo=${DEMO_MODE})`);
});
